    ...
    public Optional<CategoryEntity> findCategoryByUsernameAndSpendName(String username, String name) {
        try {
            TypedQuery<CategoryEntity> query = entityManager.createQuery(
                    "SELECT c FROM " + CategoryEntity.class.getName() + " c WHERE c.username = :username AND c.name = :name",
    ...
    public Optional<SpendEntity> findByUsernameAndSpendDescription(String username, String description) {
        try {
            TypedQuery<SpendEntity> query = entityManager.createQuery(
                    "SELECT s FROM " + SpendEntity.class.getName() + " s WHERE s.username = :username AND s.description = :description",
                    SpendEntity.class
            );
            query.setParameter("username", username);
